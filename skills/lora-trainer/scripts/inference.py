...
def check_environment():
    ...
    try:
        ...
        errors.append("[FAIL] PyTorch 未安装pip install torch")

    for lib in ["transformers", "peft"]:
        try:
            __import__(lib)
        except ImportError:
            errors.append(f"[FAIL] {lib} 未安装pip install {lib}")

    if not os.path.exists(LORA_PATH):
        errors.append(f"[FAIL] LoRA adapter 不存在: {LORA_PATH}请确认训练已完成且路径正确")
...
def format_prompt(prompt: str) -> str:
    """构造与训练时一致的 prompt 格式默认使用 instruction-output 格式"""
